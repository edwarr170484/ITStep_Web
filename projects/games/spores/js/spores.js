...
class Piece extends Base {
    ...
    checkEdge() {
        ...
        if (this.data.position.x - halfSize <= 0 && this.data.accelerations.x < 0) {
            this.data.accelerations.x *= -1;
        }
            
        if (this.data.position.x + halfSize >= SCREEN_WIDTH && this.data.accelerations.x > 0) {
            this.data.accelerations.x *= -1;
        };


        if (this.data.position.y - halfSize <= 0 && this.data.accelerations.y < 0) {
            this.data.accelerations.y *= -1;
        }

        if (this.data.position.y + halfSize >= SCREEN_HEIGHT && this.data.accelerations.y > 0){
            this.data.accelerations.y *= -1;
        }
    }
    ...
    checkCollision() {
        ...
        this.parent.data.pieces.forEach((piece) => {
            ...
            if (Base.checkCollision({
                ...
                if (this.data.size > piece.data.size) {
                    this.consume(piece);
                } else {
                    piece.consume(this);
                }
